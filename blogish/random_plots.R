...
ggplot(df3, aes(n, reward)) + geom_point() + ylim(-50, 1000) + labs(title="discrete q table, argmax")
dev.off()
...
df5 = read.delim("~/dev/drivebot/src/runs/nn_run2.hl1/stats.tsv", col.names=c("n", "length", "reward"))
...
df6 = read.delim("~/dev/drivebot/src/runs/nn_v4/stats.tsv", col.names=c("n", "length", "reward"))
df6 = head(df6, 100)
#df5$run = as.factor('nn_run2.hl1')
